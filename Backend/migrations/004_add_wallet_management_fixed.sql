-- Migration: Add wallet management support (Fixed)
-- Description: Add wallet_address column first, then wallet management features
-- Date: 2025-01-19

-- First, add the wallet_address column if it doesn't exist
ALTER TABLE users 
ADD COLUMN IF NOT EXISTS wallet_address VARCHAR(66) UNIQUE;

-- Add other wallet management columns
ALTER TABLE users 
...
COMMENT ON COLUMN users.wallet_address IS 'Aptos wallet address (66 characters with 0x prefix)';
